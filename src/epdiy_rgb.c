...
void epd_draw_hline_rgb(int x, int y, int length, rgb_pixel color,
                    rgb_pixel *framebuffer) {
  for (int i = 0; i < length; i++) {
    int xx = x + i;
    epd_draw_pixel_rgb(xx, y, color, framebuffer);
  }
}

void epd_draw_vline_rgb(int x, int y, int length, rgb_pixel color,
                    rgb_pixel *framebuffer) {
  for (int i = 0; i < length; i++) {
    int yy = y + i;
    epd_draw_pixel_rgb(x, yy, color, framebuffer);
  }
}
...
void epd_draw_pixel_rgb(int x, int y, rgb_pixel color, rgb_pixel *framebuffer) {
  ...
  rgb_pixel *buf_ptr = &framebuffer[y * epd_width() / 2 + x / 2];
  if (x % 2) {
    buf_ptr->r = (buf_ptr->r & 0x0F) | (color.r & 0xF0); //不知道这样写能不能修改framebuffer中的值
    buf_ptr->g = (buf_ptr->g & 0x0F) | (color.g & 0xF0);
    buf_ptr->b = (buf_ptr->b & 0x0F) | (color.b & 0xF0);
  } else {
    buf_ptr->r = (buf_ptr->r & 0xF0) | (color.r >> 4);
    buf_ptr->g = (buf_ptr->g & 0xF0) | (color.g >> 4);
    buf_ptr->b = (buf_ptr->b & 0xF0) | (color.b >> 4);
  }
}

void epd_draw_circle_rgb(int x0, int y0, int r, rgb_pixel color,
                     rgb_pixel *framebuffer) {
  int f = 1 - r;
  int ddF_x = 1;
  int ddF_y = -2 * r;
  ...
  epd_draw_pixel_rgb(x0, y0 + r, color, framebuffer);
  epd_draw_pixel_rgb(x0, y0 - r, color, framebuffer);
  epd_draw_pixel_rgb(x0 + r, y0, color, framebuffer);
  epd_draw_pixel_rgb(x0 - r, y0, color, framebuffer);
